...
def getAtomnr(atom) -> None:
    ...
    try:
        ...
        print(f'{atom} is not an atom or has not been implemented in AtomicInformation.getAtomnr')

def getAtom(atomnr) -> None:
    dic = {
    ...
    try:
        ...
        print(f'{atomnr} is not an atomnr or has not been implemented in AtomicInformation.getAtom')
...
class AtomicInformation():
    ...
    def getAtomnr(self) -> None:
        return getAtomnr(self.atom)

    def getAtom(self) -> None:
        return getAtom(self.atomnr)
...
class BasisSet:
    ...
    def CheckBasisSet(Program: str, BasisSet: str) -> bool:
        ...
        Bas = BasisSet
        if BasisSet == 'LANL2' or BasisSet == 'LANL2DZ_ECP':
            Bas = 'LANL2DZ ECP'
        Bas = Bas.replace(' ','%20')
    ...
    def GenerateBasisSet(self, Program: str, BasisSet: str, Atoms: list, SupressHeader: bool = False) -> str:
        ...
        if not SupressHeader:
            ...
            for i in BasisSet[:10]:
                Info += f'{i}\n'
            print("Getting basis set information from Basis Set Exchange")
            print(Info)
